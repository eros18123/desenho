...
def remove_only_drawing_html(content):
    ...
    # Remove tags de imagem antigas geradas pelo addon
...
def handle_save_message(message, context):
    try:
        ...
        img_html = parts[1]
        # clean_html_b64 = parts[2] if len(parts) > 2 else None # IGNORADO PARA PROTEGER O CLOZE
        ...
        if field_name in note:
            # CORREÇÃO AQUI:
            # Em vez de usar o HTML da tela (que tem o cloze expandido/quebrado),
            # pegamos o conteúdo original da nota, limpamos o desenho antigo e adicionamos o novo.
            original_content = note[field_name]
            cleaned_content = remove_only_drawing_html(original_content)
...
JS_SALVAR = """
    ...
    function saveDrawingToField() {
        ...
        // Mantemos a lógica de clonar apenas para não quebrar o envio, 
        // mas o Python vai ignorar o HTML gerado aqui.
        ...
        historyToSave.forEach((item) => {
            if (item.type === 'fill' || item.type === 'external_image') {
        ...
        if (hasPixels) {
            ...
            // Enviamos o cleanHtml, mas o Python agora vai ignorá-lo para proteger o Cloze
