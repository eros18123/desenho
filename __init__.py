...
if (!window.drawingCanvasInitialized) {
    ...
    drawingStyle.innerHTML = `
        /* CORREÇÃO: position absolute para acompanhar o scroll */
        #drawingCanvas { position: absolute; top: 0; left: 0; z-index: 2; pointer-events: none; }
    ...
    // CORREÇÃO: Redimensionar para o tamanho total do documento
...
JS_MOUSE_EVENTS = """
    ...
    canvas.addEventListener('dblclick', (e) => { 
        if (currentTool === 'select') { 
            ...
            if (obj) {
                if (obj.type === 'text') { 
                    selectedObject = obj; textInputBox.value = obj.text; 
                    // CORREÇÃO: Usar pageX/Y para posicionar corretamente com scroll
                    textInputBox.style.left = e.pageX + 'px'; 
                    textInputBox.style.top = e.pageY + 'px'; 
                    textInputBox.style.color = obj.color; textInputBox.style.fontSize = `${parseInt(obj.size) + 10}px`; textInputBox.style.display = 'block'; textInputBox.dataset.editingId = obj.id; setTimeout(() => textInputBox.focus(), 0); 
                } else if (obj.type === 'table') {
